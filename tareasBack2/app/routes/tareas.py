...
@router.put("/{tarea_id}")
async def actualizar_tarea(tarea_id: int, tareaUpdate: TareaUpdate, session: Session = Depends(get_session)):
    tarea : TareaDB = session.get_one(TareaDB, tarea_id)
    if not tarea:
        return {"message": "No hay tarea con este id"}
    tareaNew = tareaUpdate.model_dump(exclude_unset=True)
